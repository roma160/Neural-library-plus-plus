...
namespace NNL {
    ...
    public:
        ...
        SimpleForwardPropagationResult ForwardPropagation(const vec<double> &input_data) const;

        vec<double> ComputeNetworkAnswer(const vec<double> &input_data) const;
        ...
        void SaveToFile(std::string &file_location, bool write_in_binary) const;

        void SaveToFile(const char *file_location, bool write_in_binary) const;
    };
